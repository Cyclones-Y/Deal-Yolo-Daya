...
1.  **运行应用**：
    ...
    ```
    *注意：如果您使用 `uv`，可以运行：`uv run streamlit run src/deal_yolo_data/app.py`*

2.  **选择模式**：
    使用侧边栏在“数据处理”和“YOLO 训练平台”之间切换。

### 数据处理流程
1.  **上传数据**：上传您的原始 CSV 或 Excel 标注文件。
2.  **配置步骤**：调整每个处理步骤的设置（例如 IoU 阈值、划分比例）。
3.  **运行流水线**：按顺序执行步骤。应用会缓存中间结果，允许您重新运行特定步骤而无需从头开始。
4.  **下载结果**：下载处理后的数据集、中间 CSV 文件或最终的 YOLO 格式压缩包。

### 训练流程
1.  **选择数据集**：选择包含 `data.yaml` 文件的相关数据集目录。
2.  **配置训练**：设置模型路径、训练轮数（Epochs）和其他参数。
3.  **开始训练**：点击“开始训练”。您可以实时监控日志。
4.  **分析结果**：训练结束后，选择结果目录以查看指标和图表。

## 项目结构

*   `src/deal_yolo_data/app.py`: Streamlit 应用主入口文件。
